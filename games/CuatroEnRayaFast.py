...
class CuatroEnRayaFast(BaseGame):
    ROWS = 6
    COLS = 7
    ACTION_SIZE = COLS
    ...
    def __init__(self):
        ...
        self.player = 1
        self.result = None
        self.top = np.full(self.COLS, self.ROWS - 1, dtype=np.int8)
        self.move_history = []
    ...
    def clone(self):
        cloned = CuatroEnRayaFast()
        cloned.board = self.board.copy()
        cloned.player = self.player
        cloned.result = self.result
        cloned.top = self.top.copy()
        cloned.move_history = self.move_history.copy()
        return cloned
